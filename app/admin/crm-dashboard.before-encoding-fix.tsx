...
export default function CrmDashboard({
    ...
    async function loadLeadCard() {
      ...
      try {
        ...
        if (!response.ok || !result.ok) {
          setNotice(result.error || "е удалось загрузить карточку клиента");
          return;
        }
        ...
        if (!cancelled) {
          setNotice(
            error instanceof Error
              ? error.message
              : "шибка загрузки карточки клиента"
          );
        }
      } finally {
        if (!cancelled) {
  ...
  try {
    ...
    if (!response.ok || !result.ok) {
      setNotice(result.error || "шибка AI-анализа");
      return;
    }
    ...
    if (!saveResponse.ok || !saveResult.ok) {
      setNotice(saveResult.error || "е удалось сохранить AI-анализ");
      return;
    }

    setSelectedLead(saveResult.lead ?? {
      ...selectedLead,
      ai_score: ai.score ?? 0,
         ...
         <section className="clientCard">
           {leadCardLoading ? (
             <div className="clientCardLoading">
               агрузка карточки клиента...
             </div>
           ) : selectedLead ? (
             <>
               <div className="clientCardHeader">
                 <div>
                   <div className="clientCardEyebrow">Т</div>
                   <h2>{selectedLead.name}</h2>
                   <p>
                     {selectedLead.company || "ез компании"} ·{" "}
                     {selectedLead.niche || "иша не указана"}
                   </p>
                 </div>
              ...
              <div className="clientCardSection">
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:"12px"}}>
                  ...
                  >
                    {aiCardLoading ? "AI анализирует..." : "апустить AI-анализ"}
                  </button>
                </div>
